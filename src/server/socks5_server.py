...
class SOCKS5Server:
    ...
    def __init__(self, host="0.0.0.0", port=const.DEFAULT_SERVER_PORT):
        """
    ...
    def _socks5_initialization(self, client_socket: socket.socket) -> bool:
        ...
        try:
            ...
            if const.AUTH_NO_AUTH not in methods:
    ...
    def _socks5_request(self, client_socket: socket.socket) -> Optional[socket.socket]:
        """
        ...
        try:
            ...
            if version != const.SOCKS_VERSION:
                logger.error(f"Unsupported SOCKS5 version: {version}")
                client_socket.sendall(
                    utils.create_socks_reply_packet(const.REPLY_GENERAL_FAILURE)
                )
                return None

            if cmd != const.CMD_CONNECT:  # Only support CONNECT command
    ...
    def _connect_to_destination(self, client_socket, dest_addr, dest_port):
        """
